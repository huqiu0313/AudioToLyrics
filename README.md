...
- 🔍 **联网搜索歌词** — 优先从 QQ音乐、酷狗、LRCLib、网易云搜索官方歌词（v5.1：搜索关键词优化为"歌名 歌手"格式）
- 🎼 **专辑封面下载** — 自动搜索专辑信息并下载封面写入音频文件（v5.1：已有封面/专辑则不覆盖）
- 📝 **音频 tag 写入** — 自动写入标题/艺术家/专辑/封面（支持 MP3/FLAC/M4A/OGG）
...
- 🔄 **繁简转换** — 繁体中文自动转简体
- 📐 **相似度校验** — 防止歌词误匹配（标题+歌手双重校验，v5.1：修复多歌手匹配bug）
...
- 🚨 **失败文件保留** — 处理失败的文件保留在列表中并显示详细原因（v5.1 新增）
...
>
> **v5.1**：支持 OGG 格式的 tag 读写与封面嵌入；多歌手名自动使用 `/` 分隔以正确匹配。
...
| [unlock music](https://git.um-react.app/um/um-react) | 如遇解码音乐格式等问题可访问：https://unlock-music.liumingye.cn/questions
...
- **搜索关键词优化**：搜索词从 "歌手 歌名" 统一为 "歌名 歌手"，提高匹配准确率
- **多歌手匹配修复**：修复QQ音乐/网易云多歌手名用空格拼接导致匹配失败的bug，统一使用 `/` 分隔
- **OGG 格式完整支持**：新增 OGG 音频的 tag 写入（标题/艺术家/专辑/封面）及元数据读取
- **智能标签保护**：已有专辑名或封面图的文件不再重复下载和覆盖写入
- **失败文件保留**：处理失败的文件保留在文件列表中，状态栏显示详细错误原因（如"歌词匹配失败，匹配关键词为xxx"）
- **冗余代码清理**：移除未定义变量的时长校验函数、重复的死代码分支
